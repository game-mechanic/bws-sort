...
namespace DT.GridSystem
{
    ...
    public abstract class GridSystem<TGridObject> : MonoBehaviour
    {
        ...
        /// <summary>
        /// Guard flag set to <c>true</c> while the grid array is being reallocated
        /// (e.g. inside <see cref="ResizeGridIfNeededEditor"/>). Any method that
        /// reads or writes <see cref="gridArray"/> should early-exit when this is set
        /// to avoid <see cref="System.IndexOutOfRangeException"/> during OnValidate.
        /// </summary>
        ...
        /// </summary>
        public virtual Vector2 CellSize { get => cellSize; protected set => cellSize = value; }
        ...
        /// </summary>
        public void SetUpGrid(Vector2Int size, Vector2 cellsize)
        {
            // -- Validation --
        ...
        {
            if (_isResizing) { Debug.LogWarning("[GridSystem] AddGridObject called while the grid array is being resized. Operation skipped."); return; }
        ...
        {
            if (_isResizing) { Debug.LogWarning("[GridSystem] RemoveGridObject called while the grid array is being resized. Operation skipped."); return default; }
            if (x >= 0 && y >= 0 && x < GridSize.x && y < GridSize.y)
            {
                int index = ToIndex(x, y);
        ...
        {
            if (_isResizing) { Debug.LogWarning("[GridSystem] GetGridObject called while the grid array is being resized. Returning default."); return default; }
            if (x >= 0 && y >= 0 && x < GridSize.x && y < GridSize.y)
            {
                int index = ToIndex(x, y);
        ...
        private void DrawIndexes()
        {
            ...
            if (gridArray == null || gridArray.Length < ToIndex(GridSize.x - 1, GridSize.y - 1) + 1)
                return;
        ...
        public virtual void ResizeGridIfNeededEditor()
        {
            ...
            if (previousGridSize == Vector2Int.zero)
            {
                ...
                // Component was just added — gridArray may never have been allocated.
                // Initialize it here so callers never see a null array.
                int initialSize = gridSize.x * gridSize.y;
                if (gridArray == null || gridArray.Length != initialSize)
                {
                    UnityEditor.Undo.RegisterCompleteObjectUndo(this, "Initialize Grid Array");
                    gridArray = new TGridObject[initialSize];
        ...
        {
            if (_isResizing) { Debug.LogWarning("[GridSystem] ClearGrid called while the grid array is being resized. Operation skipped."); return; }
        ...
        {
            if (_isResizing) { Debug.LogWarning("[GridSystem] ForEachCell called while the grid array is being resized. Operation skipped."); return; }
            for (int x = 0; x < GridSize.x; x++)
                for (int y = 0; y < GridSize.y; y++)
        ...
        {
            if (_isResizing) { Debug.LogWarning("[GridSystem] GetOrNull called while the grid array is being resized. Returning default."); return default; }
        ...
        {
            if (_isResizing) { Debug.LogWarning("[GridSystem] Each() called while the grid array is being resized. Returning empty enumerable."); yield break; }
            for (int x = 0; x < GridSize.x; x++)
                for (int y = 0; y < GridSize.y; y++)
