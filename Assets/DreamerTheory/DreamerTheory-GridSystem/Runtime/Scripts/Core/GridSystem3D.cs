    ...
    public class GridSystem3D<TGridObject> : RectGridSystem<TGridObject>
    {
        ...
        public override Vector3 GetWorldPosition(int x, int y, bool snapToGrid = true)
        {
            ...
            Vector3 localPosition = new Vector3(
                (x - GridSize.x / 2f) * CellSize.x,
                0f,
                (y - GridSize.y / 2f) * CellSize.y
            );

            if (snapToGrid)
            {
                localPosition += new Vector3(CellSize.x, 0f, CellSize.y) * 0.5f;
            }
        ...
        public override void GetGridPosition(Vector3 worldPosition, out int x, out int y)
        {
            ...
            float relativeX = localPosition.x / CellSize.x;
            float relativeY = localPosition.z / CellSize.y;
            ...
            for (int x = 0; x < GridSize.x; x++)
            {
                for (int y = 0; y < GridSize.y; y++)
                {
                    if (TryGetGridObject(x, y, out _))
                    {
                        ...
                        Gizmos.DrawSphere(objectPosition, 0.1f);
                    }
                }
            }
#endif
