...
namespace DT.GridSystem
{
    public class HexGridSystem<TGridObject> : GridSystem<TGridObject>
    {
        ...
        public override Vector2 CellSize
        {
            get
            {
                return new(base.CellSize.x, base.CellSize.x);
            }

            protected set
            {
                base.CellSize = new(value.x, value.x);
            }
        }
            ...
            switch (hexOrientation)
            {
                case HexOrientation.PointyTop:
                    ...
                    if (plane == GridPlane.XZ)
                        localPosition = new Vector3(xPos, 0, yPos) + new Vector3(CellSize.x, 0, CellSize.x) * 0.5f;
                    else
                        localPosition = new Vector3(xPos, yPos, 0) + new Vector3(CellSize.x, CellSize.x, 0) * 0.5f;
        ...
        public override void GetGridPosition(Vector3 worldPosition, out int x, out int y)
        {
            ...
            if (plane == GridPlane.XZ)
                offsetCenter = new Vector3(CellSize.x, 0, CellSize.x) * 0.5f;
            else
                offsetCenter = new Vector3(CellSize.x, CellSize.x, 0) * 0.5f;
        ...
        public override void OnDrawGizmos()
        {
            ...
            for (int i = 0; i < GridSize.x; i++)
            {
                for (int j = 0; j < GridSize.y; j++)
                {
                    ...
                    DrawHexOutline(position, CellSize.x * 0.45f);
                    if (showGridIndex)
