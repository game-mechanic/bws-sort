namespace Dreamteck.Splines.Editor
{
    ...
    public static class SplineEditorHandles
    {
        ...
        public static bool Slider(SplineComputer spline, ref double percent, Color color, string text = "", SplineSliderGizmo gizmo = SplineSliderGizmo.Rectangle, float buttonSize = 1f)
        {
            ...
            var fmh_78_79_639125565302071546 = Quaternion.LookRotation(cam.transform.position - evalResult.position); evalResult.position = Handles.FreeMoveHandle(evalResult.position, size * 0.2f * buttonSize, Vector3.zero, Handles.CircleHandleCap);
        ...
        {
            var fmh_123_53_639125565302077731 = Quaternion.identity; return Handles.FreeMoveHandle(position, size, Vector3.zero, Handles.CircleHandleCap);
        }

        public static Vector3 FreeMoveCircle(Vector3 position, float size)
        {
            var fmh_128_53_639125565302082214 = Quaternion.identity; return Handles.FreeMoveHandle(position, size, Vector3.zero, Handles.CircleHandleCap);
        }
