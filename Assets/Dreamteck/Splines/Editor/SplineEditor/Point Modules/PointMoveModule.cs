namespace Dreamteck.Splines.Editor
{
    ...
    public class PointMoveModule : PointTransformModule
    {
        ...
        public override void DrawScene()
        {
            ...
            if (surfaceMode)
            {
                var fmh_81_47_639125565301917384 = Quaternion.LookRotation(SceneView.currentDrawingSceneView.camera.transform.position - c); c = Handles.FreeMoveHandle(c, HandleUtility.GetHandleSize(c) * 0.2f, Vector3.zero, Handles.CircleHandleCap);
                if(lastPos != c)
                {
